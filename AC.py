...
class Critic(object):
    def __init__(self, sess, n_features,n_actions, lr=0.01):
        ...
        self.a_onehot = tf.squeeze(self.a_onehot,axis=1) #(?, 2)
        # print(self.a_onehot.shape)
        # exit()

        self.input = tf.concat([self.s,self.a_onehot],axis=1) #(?,6)
        # print(self.input)
        # exit()
    ...
    def learn(self, s, a, r, s_):
        ...
        next_a = [[i] for i in range(N_A)] #[[0], [1]]
        # print(next_a)
        # exit()
        s_ = np.tile(s_,[N_A,1])  #[2,4]
        # print(s_.shape)
        # exit()
...
for i_episode in range(MAX_EPISODE):
    ...
    while True:
        ...
        if done or t >= MAX_EP_STEPS:
            ...
            print("episode:", i_episode, "  reward:", int(running_reward),"alive time: ",alive_time)
            alive_time = 0
